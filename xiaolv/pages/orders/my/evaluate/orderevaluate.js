...
const saveCarwashCommentsUrl = config.saveCarwashCommentsUrl
const saveCommentsUrl = config.saveCommentsUrl
var app = getApp()
...
Page({
  ...
  commentRequest:function(e){
    ...
    if (this.data.entity.orderType == 1){
      util.kmRequest({
        data: {
          interfaceName: saveCommentsUrl,
          param:{
            orderId: this.data.entity.id,
            comment: e.detail.value.content,
            stars: this.data.key1,
          }
        },
        method:"post",
    ...
    if (this.data.entity.orderType == 0){
      util.kmRequest({
        data:{
    ...
    if (this.data.entity.orderType == 2){
    util.kmRequest({
      data: {
        interfaceName: saveCarwashCommentsUrl,
        param:{
          orderId: this.data.entity.id,
          comment: e.detail.value.content,
          stars1: this.data.key + 1,
          stars2: this.data.key1 + 1,
          stars3: this.data.key + 1
        }
      },
      success: function (res) {
        if(res.data.status == 1){
